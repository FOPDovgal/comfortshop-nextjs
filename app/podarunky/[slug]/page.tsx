...
import { getEntityPage } from "@/lib/entity-pages";
...
import type { DiscoverSource } from "@/lib/discover-pages";
...
export async function generateMetadata({
  ...
  const config = getEntityPage(slug);
  if (!config) return {};
  ...
  const ogImageUrl = await resolveImage("entity", slug, articleImageUrl);

  return {
    title: config.seo_title,
    description: config.seo_description,
    robots: config.status === "draft" ? { index: false } : undefined,
    alternates: { canonical: `https://comfortshop.com.ua/podarunky/${slug}` },
    openGraph: {
      type: "website",
      title: config.seo_title,
      description: config.seo_description,
      url: `https://comfortshop.com.ua/podarunky/${slug}`,
...
async function aggregateArticles(sources: DiscoverSource): Promise<Article[]> {
...
export default async function GiftEntityPage({
  params,
}: {
  params: Promise<{ slug: string }>;
}) {
  ...
  const minArticles = config.min_articles ?? 3;
  ...
  // Parent category colors for gradient (optional)
  const parentCat = config.parent_category ? getCategoryBySlug(config.parent_category) : null;
  const colorFrom = parentCat?.colorFrom ?? "#f97316";
  const colorTo = parentCat?.colorTo ?? "#fb923c";

  // Related discover pages — only render slugs that actually exist in DISCOVER_PAGES
  const relatedPages = (config.related_discover ?? [])
    .map((s) => DISCOVER_PAGES.find((p) => p.slug === s && p.status === "published"))
    .filter((p): p is NonNullable<typeof p> => p !== undefined);

  // JSON-LD — /podarunky index does not exist yet; omit `item` from position 2
  const breadcrumbLd = {
    "@context": "https://schema.org",
    "@type": "BreadcrumbList",
    itemListElement: [
      { "@type": "ListItem", position: 1, name: "Головна", item: "https://comfortshop.com.ua" },
      { "@type": "ListItem", position: 2, name: "Подарунки", item: "https://comfortshop.com.ua/podarunky" },
      { "@type": "ListItem", position: 3, name: config.title, item: `https://comfortshop.com.ua/podarunky/${slug}` },
    ],
  };
  ...
  return (
    <main className="mx-auto max-w-7xl px-4 py-10">
      ...
      <nav className="mb-6 flex items-center gap-2 text-sm text-gray-400">
        ...
        <span>›</span>
        <Link href="/podarunky" className="hover:text-gray-600">Подарунки</Link>
        <span>›</span>
      ...
      >
        ...
        <div className="relative z-10">
          <p className="mb-3 text-5xl drop-shadow-md">🎁</p>
